...
module.exports={
     ...
     entry:{
        ...
        index:'./src/index.js',
        mylink:'./src/link.js', //键值key 为文件名，值为源文件名字包含路径
    },
    output:{
        // 出口
        // filename:'build.js',//重新取名字
        // filename:'build.[hash:8].js',
        filename:'[name].[hash:8].js',//-此方法是多页配置出口 但这样就必须修改模板页配置plugins>HtmlWebpackPlugin
    ...
    plugins:[
        ...
        new HtmlWebpackPlugin({
            ...
            chunks:['index'],//如果不写就会所有html页面都会引入同样的js文件，不做区分
        }),
        new HtmlWebpackPlugin({
            filename:'link.html',
            template:'./src/newlink.html',
            hash: true,
            chunks:['mylink']//单独为引入link,并且注意`入口`文件名字已经被重新定义为mylink，引入src 的abc文件中的js，并且打包后filename把文件改成了link.html
        }),
